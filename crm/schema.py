...
STATUSES = ('New', 'Contacted', 'Followed up', 'Responded',
            'Pre-screened', 'Shortlisted', 'Declined', 'Bounced')
...
def is_handcraft_required(lead: dict) -> bool:
    ...
    if archetype == 'criminal' and source in ('nonprofit', 'org_intro'):
        return True
    if source == 'local_news' and lead.get('Contact Method') == 'journalist_intro':
        return True
    if source in ('storycorps', 'annual_report'):
        return True
    if status == 'Responded':
        return True
